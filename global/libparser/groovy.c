...
groovy(const struct parser_param *param)
{
  ...
  *classname = *completename = 0;
  ...
  while ((c = nexttoken(interested, groovy_reserved_word)) != EOF) {
    switch (c) {
      case SYMBOL:  /* symbol */
        for (; c == SYMBOL && peekc(1) == '.'; c = nexttoken(interested, groovy_reserved_word)) {
      ...
      case ';':
        ...
        break;
      case GROOVY_CLASS:
      case GROOVY_INTERFACE:
      case GROOVY_ENUM:
        if ((c = nexttoken(interested, groovy_reserved_word)) == SYMBOL) {
        ...
        break;
      case GROOVY_NEW:
      case GROOVY_INSTANCEOF:
        while ((c = nexttoken(interested, groovy_reserved_word)) == SYMBOL && peekc(1) == '.')
          PUT(PARSER_REF_SYM, token, lineno, sp);
        if (c == SYMBOL)
          PUT(PARSER_REF_SYM, token, lineno, sp);
        break;
      ...
      case GROOVY_VOID:
      case GROOVY_DEF:
        if (peekc(1) == '.' && (c = nexttoken(interested, groovy_reserved_word)) != GROOVY_CLASS)
          pushbacktoken();
        break;
      default:
        break;
